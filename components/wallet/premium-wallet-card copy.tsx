...
export function PremiumWalletCard({ balance, cardType, holderName }: PremiumWalletCardProps) {
  ...
  return (
    <div className="perspective-1000">
      <motion.div
        className="relative w-full h-60 transform-style-preserve-3d cursor-pointer"
        animate={{ rotateY: isFlipped ? 180 : 0 }}
        transition={{ duration: 0.6, type: "spring" }}
        onClick={() => setIsFlipped(!isFlipped)}
      >
        {/* Front of Card */}
        <Card className="absolute inset-0 backface-hidden py-0 bg-gray-700 material-card-floating border-0 overflow-hidden">
          <CardContent className="p-0 h-full">
            <div className={`${config.gradient} h-full p-6 text-white relative overflow-hidden texture-carbon`}>
              {/* Holographic Effect */}
              <div className="absolute top-4 right-4 w-16 h-12 bg-gradient-to-r from-white/20 to-transparent rounded-lg opacity-60"></div>
              ...
              <div className="relative z-10 h-full flex flex-col justify-between">
                ...
                {/* Logo */}
                <div className="absolute" style={{ bottom: "4.7rem", right: "0.5rem" }}>
                  <div
        ...
        {/* Back of Card */}
        <Card className="absolute inset-0 backface-hidden rotate-y-180 material-card-floating border-0 overflow-hidden">
          <CardContent className="p-0 h-full">
            <div className={`${config.gradient} h-full p-6 text-white relative overflow-hidden`}>
              {/* Magnetic Strip */}
